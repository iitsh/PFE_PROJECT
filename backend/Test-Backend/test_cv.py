...
# ── Configuration JWT pour les tests (même valeurs que dans mock_env) ─────
...
def mock_env():
    ...
    Patch à la fois os.environ ET les variables déjà lues par le module auth."""
    with patch.dict(os.environ, {
        ...
        # Patch aussi les variables déjà lues au chargement du module auth
        import routes.auth as auth_mod
        original_key = auth_mod.JWT_KEY       # Sauvegarde les valeurs originales
        original_algo = auth_mod.JWT_ALGORITHM
        original_issuer = auth_mod.JWT_ISSUER
        original_audience = auth_mod.JWT_AUDIENCE
        auth_mod.JWT_KEY = JWT_KEY            # Remplace par les valeurs de test
        auth_mod.JWT_ALGORITHM = JWT_ALGORITHM
        auth_mod.JWT_ISSUER = JWT_ISSUER
        auth_mod.JWT_AUDIENCE = JWT_AUDIENCE
        yield # Exécute le test
        # Restaure les valeurs originales après le test
        auth_mod.JWT_KEY = original_key
        auth_mod.JWT_ALGORITHM = original_algo
        auth_mod.JWT_ISSUER = original_issuer
        auth_mod.JWT_AUDIENCE = original_audience
...
def mock_db():
    ...
    with patch("routes.cv.get_db") as mock_cv, \
         patch("routes.auth.get_db") as mock_auth:
...
class TestGenerateCV:
    ...
    def test_generation_reussie(self, mock_pipeline, mock_db):
        ...
        mock_pipeline.construire_html_lettre.return_value = "<html>Lettre</html>" # HTML lettre
        mock_pipeline.renderiser_cv_html.return_value = "<html>CV</html>" # HTML CV
        ...
        assert "<html>CV</html>" in data["html_cv"] # Le HTML du CV est présent
