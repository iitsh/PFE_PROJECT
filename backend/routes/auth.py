...
# ── Configuration JWT (lue depuis les variables d'environnement) ─────────────
# Clé secrète pour signer les tokens (DOIT être sécurisée et unique)
JWT_KEY                      = os.getenv("JWT_KEY")
# Algorithme de signature (HS256 par défaut)
JWT_ALGORITHM                = os.getenv("JWT_ALGORITHM", "HS256")
# Émetteur du token (identifie qui a créé le token)
JWT_ISSUER                   = os.getenv("JWT_ISSUER", "backend")
# Audience du token (identifie qui doit consommer le token)
JWT_AUDIENCE                 = os.getenv("JWT_AUDIENCE", "pfe_frontend")
# Durée de validité de l'access token en minutes (court : 1 min par défaut)
JWT_DURATION_MINUTES         = int(os.getenv("JWT_DURATION_IN_MINUTES", 1))
# Durée de validité du refresh token en minutes (plus long : 3 min par défaut)
...
def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(bearer_scheme)):
    try:
        ...
        return jwt.decode(
            credentials.credentials, JWT_KEY,
...
def generate_access_token(email: str) -> tuple[str, int]:
    ...
    token = jwt.encode({
        ...
        "iss": JWT_ISSUER, "aud": JWT_AUDIENCE, "nbf": now
...
def generate_tokens(email: str) -> dict:
    ...
    refresh_exp = now + timedelta(minutes=JWT_REFRESH_DURATION_MINUTES)
...
def login(data: LoginData, response: Response, request: Request):
    ...
    response.set_cookie(
        ...
        max_age=JWT_REFRESH_DURATION_MINUTES * 60, path="/"
    )
